...
import { grantGroupStagePredictionExtension } from "@/app/actions/god";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

export function GodExtensionForm({
  groupId,
  userId,
  label
}: {
  ...
  return (
    <form action={grantGroupStagePredictionExtension} className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
      <input type="hidden" name="groupId" value={groupId} />
      {userId ? <input type="hidden" name="userId" value={userId} /> : null}
      <input type="hidden" name="expiresAtIso" value={expiresAtIso} />
      <Input
        aria-label={`${label} deadline`}
        type="datetime-local"
        value={localDeadline}
